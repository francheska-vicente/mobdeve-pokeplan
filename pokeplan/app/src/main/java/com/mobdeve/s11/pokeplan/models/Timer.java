...
public class Timer {
    ...
    /**
     * Converts the time stored in the timer to minutes
     * @return the time stored in the timer in minutes
     */
    public int convertToMinutes() {
        if (secs > 0)
            return (int) TimeUnit.HOURS.toMinutes(hours) + mins + 1;
        return (int) TimeUnit.HOURS.toMinutes(hours) + mins;
    }
}
